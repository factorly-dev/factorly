...
When Factorly runs inside a container, use `host.docker.internal` from the host or bind to `0.0.0.0`:
...
1. `factorly serve --http :3000` starts an HTTP server at `/mcp` using the MCP Streamable HTTP transport.
...
3. All configured tools are exposed over HTTP. Built-in tools (`factorly.shell`, `factorly.write_file`, etc.) are hidden in HTTP mode since local operations don't make sense on a remote server — only `factorly.fetch` remains available.
